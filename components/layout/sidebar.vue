<template>
  <ui-bordered-box>
    <p class="font-bold mb-2">Concepts</p>
    <ul class="ml-4 list-disc list-inside">
      <li><a href="" class="hover:underline">What's Tailwind CSS?</a></li>
      <li><a href="" class="hover:underline">Utility-first CSS</a></li>
    </ul>

    <p class="font-bold mt-6 mb-2">Commonly Used</p>
    <ul class="ml-4 list-disc list-inside">
      <li><a href="" class="hover:underline">Typography</a></li>
      <li><a href="" class="hover:underline">Colors</a></li>
      <li><a href="" class="hover:underline">Borders</a></li>
      <li><a href="" class="hover:underline">Sizing</a></li>
      <li><a href="" class="hover:underline">Spacing</a></li>
      <li><a href="" class="hover:underline">Breakpoints</a></li>
      <li><a href="" class="hover:underline">States</a></li>
    </ul>

    <p class="font-bold mt-6 mb-2">Advance CSS concepts</p>
    <ul class="ml-4 list-disc list-inside">
      <li><a href="" class="hover:underline">Inline vs Block</a></li>
      <li><a href="" class="hover:underline">Flexbox</a></li>
      <li><a href="" class="hover:underline">Grid</a></li>
    </ul>
  </ui-bordered-box>
</template>
